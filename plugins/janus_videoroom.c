...
static void janus_videoroom_enable_substreams(gpointer key, gpointer value, gpointer user_data) {
    ...
    if(!session || !session->handle || session->participant_type != janus_videoroom_p_type_publisher) {
        return;
    }
    ...
    if(publisher) {
        ...
        JANUS_LOG(LOG_INFO, "[samvel]{%s:%d} %p : len=%d (%d, %d, %d)\n", __FUNCTION__, __LINE__, publisher->subscribers, length, publisher->ssrc[0], publisher->ssrc[1], publisher->ssrc[2]);
...
static void janus_videoroom_max_substreams_calc() {
    ...
    if(prev_max_substream != max_substream) {
...
static uint32_t  janus_videoroom_get_next_target(const janus_videoroom_publisher* publisher, const janus_videoroom_subscriber *subscriber) {
...
static void janus_videoroom_enable_streams(janus_videoroom_session *session, janus_videoroom_publisher* publisher) {
    ...
    if(!session || !session->handle || !subscribers) {
        return;
    }
    ...
    while (subscribers) {
        ...
        if(!subscriber || !subscriber->session || !subscriber->video || subscriber->paused || subscriber->kicked) {
            continue;
        }
